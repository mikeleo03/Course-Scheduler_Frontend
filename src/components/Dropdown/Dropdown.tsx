...
const Dropdown: React.FC<DropdownProps> = ({ menuItems, selectedItem, setSelectedItem, height, maxHeight }) => {
    ...
    return (
        <>
            <button
                /* ref={selectMenuRef} */
                className="flex items-center gap-2 w-full pr-2.5 pl-4 py-1.5 mt-2 text-gray-500 bg-white border rounded-lg shadow-sm cursor-default outline-none focus:border-indigo-600"
                aria-haspopup="true"
                aria-expanded="true"
            ...
            {state && (
                <div className="relative w-full">
                    <ul
                        className={`absolute w-full mt-3 overflow-y-auto bg-white border rounded-md shadow-sm ${height !== 0 ? `h-${height}` : ""} ${maxHeight !== 0 ? `max-h-${maxHeight}` : ""}`}
                        role="listbox"
                    >
